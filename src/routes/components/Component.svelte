        ...
        <div class="content">
            ...
            <div class="actions">
                ...
                {#if label == 'Media Gallery'}
                    <button class="add-component colour-success" class:disabled={baseContainer.components.find(c => c.id == id + 1)?.items?.length == 10} disabled={baseContainer.components.find(c => c.id == id + 1)?.items?.length == 10} on:click={() => {
                        baseContainer.components = baseContainer.components.map((component) => {
                            if (component.type === ComponentType.MediaGallery) {
                                // @ts-ignore
                                component.items = [...(component.items ?? []), { media: { url: "https://media.piny.dev/DaalBotSquare.png", hex_id: Math.floor(Math.random() * 1000) } }];
                            }
                            return component;
                        });
                ...
                {#if label == 'Action Row'}
                    <button class="add-component colour-success"
                    class:disabled={baseContainer.components.find(c => c.id == id + 1)?.components?.length == 5}
                    disabled={baseContainer.components.find(c => c.id == id + 1)?.components?.length == 5}
                    on:click={() => {
                        const existingChildren = baseContainer.components.find(c => c.id == id + 1)?.components;

                        // Check if the component is already at 1 selector
                        if (existingChildren?.length == 1 && existingChildren[0].type != ComponentType.Button) {
                        ...
                        mount(TypePicker, {
                            ...
                            props: {
                                allowedTypes: ((existingChildren ?? []).filter(c => c.type == ComponentType.Button).length >= 1) 
                                ...
                                submit: (ic: Component) => {
                                    ...
                                    const newComponent = baseContainer.components.find(c => c.id == id + 1)?.components?.slice(-1)[0];
                ...
                {#if !noClose}
                <button class="close-component colour-secondary" on:click={() => {setTimeout(() => {expanded = false}, 100)}}>Close</button>
                {/if}
                {#if !noDelete}
                <button class="remove-component colour-danger" on:click={() => {
                    components.update((c) => {
                        baseContainer.components = baseContainer.components.filter((component) => component.id !== id + 1);
                        return c;
                    });
                ...
                {#each customButtons as button}
                    <button class="add-component {button.class}" on:click={() => button.onClick()}>{button.label}</button>
                {/each}
            </div>
        </div>
    {/if}

    ...
    <div class="container">
        ...
        <select id="component-type">
            {#each Object.entries(typeNames) as [type, name]}
                {#if allowedTypes.includes(parseInt(type))} <!-- Containers would be a lot more effort, maybe in the future -->
        ...
        <div class="actions">
            ...
            <button class="add-component colour-success" on:click={() => {
                const selectedType = (document.querySelector('#component-type') as HTMLSelectElement).value;

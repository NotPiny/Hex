...
<button class="action-button colour-{buttonStyleMapping[comp.style ?? 1]}{comp.disabled ? ' disabled' : ''}">
    <div class="button-content">
        {#if comp.emoji}
            {#if comp.emoji.length > 2} <!-- Custom emoji id -->
...
<div class="message-contents" style="{baseContainer.accent_color ? `border-left: 5px solid ${baseContainer.accent_color};` : ''}">
    ...
    {#each baseContainer?.components || [] as component}
        ...
        {#if component.type === ComponentType.File}
            <div class="file">
                <img src={dev ? `/icons/components/${ComponentType.File}.svg` : `https://cdn.jsdelivr.net/gh/NotPiny/Hex/static/icons/components/${ComponentType.File}.svg`} alt="File Icon" />
                <div class="details">
